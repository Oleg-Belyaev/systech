...
var dataConvert = [];
var numberOfDateStart = 8;
var numberOfDateEnd = 10;
var numberOfMonthStart = 5;
var numberOfMonthEnd = 7;
var day = 0;
...
for(var i = 0; i < dataConvert.length; i++) {
  if (i === 0) {
  ...
  } else if (i > 0 && i < dataConvert.length - 1 && dataConvert[i].date.slice(0, numberOfDateEnd) === dataConvert[i - 1].date.slice(0, numberOfDateEnd)) {
    if (dataConvert[i].id !== dataConvert[i-1].id) {
      documentCount++;
    }
    documentCost += (dataConvert[i].price * dataConvert[i].quantity)
  } else if (i > 0 && i < dataConvert.length - 1 && dataConvert[i].date.slice(0, numberOfDateEnd) !== dataConvert[i - 1].date.slice(0, numberOfDateEnd)) {
    days.push({day: (dataConvert[i-1].date.slice(numberOfDateStart, numberOfDateEnd))+' '+month[dataConvert[i-1].date.slice(numberOfMonthStart, numberOfMonthEnd)], documentCount: documentCount, documentCost: documentCost});
    documentCount = 0;
    documentCost = 0;
    documentCount++;
    documentCost += (dataConvert[i].price * dataConvert[i].quantity)
    ...
    documentCost += (dataConvert[i].price * dataConvert[i].quantity)
    days.push({day: (dataConvert[i].date.slice(numberOfDateStart, numberOfDateEnd))+' '+month[dataConvert[i].date.slice(numberOfMonthStart, numberOfMonthEnd)],documentCount: documentCount, documentCost: documentCost});
  }
};
...
function addToDocuments () {
  documents.push({
    ...
    day: day,
    date: dataConvert[i - 1].date.slice(0, numberOfDateEnd),
...
for(var i = 0; i < dataConvert.length; i++) {
  if (i === 0) {
    ...
    if (documents.length !== 0) {
      if (dataConvert[i - 1].date.slice(0, numberOfDateEnd) !== documents[documentPosition - 1].date) {
        day++ 
      }
    }
    addToDocuments();
...
function convertNumber (number) {
  return Number(number).toFixed(2).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ').replace('.', ',');
}
...
for(var i = 0; i < days.length; i++) {
  ...
  dayElement.querySelector('.day__description').textContent = 'Документов: ' + days[i].documentCount + ' (' + convertNumber(days[i].documentCost) + ' ₽)';
...
for(var i = 0; i < documents.length; i++) {
  ...
  documentElement.querySelector('.document__cost').textContent = convertNumber(documents[i].productCost) + ' ₽';
...
for(var i = 0; i < dataConvert.length; i++) {
  ...
  productElement.querySelector('.product__price').textContent = dataConvert[i].quantity + ' штук х ' + convertNumber(dataConvert[i].price) + ' ₽';
  productElement.querySelector('.product__cost').textContent = convertNumber(dataConvert[i].quantity * dataConvert[i].price) + ' ₽';
...
var dayDateNodeList = document.querySelectorAll('.day__date');
...
function addAttributeForDay (pos) {
  ...
  dayContainerNodeList[i].addEventListener('click', function () {
    if (dayDocumentNodeList[i].hasAttribute('style')) {
      var text = dayDateNodeList[i].textContent;
      var newText = text.replace('▲', '▼')
      dayDateNodeList[i].textContent = newText;
...
function addAttributeForDocument (pos) {
  ...
  documentContainerNodeList[i].addEventListener('click', function () {
    if (documentDocumentNodeList[i].hasAttribute('style')) {
      var text = documentContainerNodeList[i].textContent;
      var newText = text.replace('▲', '▼')
      documentContainerNodeList[i].textContent = newText;
